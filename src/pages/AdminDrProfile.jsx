...
// Mock data — replace with a real fetch keyed by date once the backend is ready
const DAYS = [
  { day: "Sat", date: 1 },
  { day: "sun", date: 2 },
  { day: "Mon", date: 3 },
...
const APPOINTMENTS_BY_DATE = {
  1: [
    { name: "Omar Khaled", note: "Routine Checkup", time: "10:00 Am" },
    { name: "Lina Nasser", note: "Follow-up Visit", time: "11:30 Am" },
  ],
  2: [],
  3: [
    { name: "Sarah Ali", note: "Chest Pain Evaluation", time: "9:00 Am" },
    { name: "Sarah Ali", note: "Chest Pain Evaluation", time: "9:00 Am" },
    { name: "Sarah Ali", note: "Chest Pain Evaluation", time: "9:00 Am" },
    { name: "Sarah Ali", note: "Chest Pain Evaluation", time: "9:00 Am" },
  ],
  4: [{ name: "Mona Adel", note: "ECG Review", time: "1:00 Pm" }],
};
  ...
  return (
    <button
      ...
      className={`
        ...
        transition-colors
        ${active ? "bg-[#0018A6] text-white" : "text-[#A7ABB5] hover:bg-[#F5F6F8]"}
      `}
    >
      <span>{day}</span>
...
// Generic modal shell used by both Edit profile and Message
...
function DoctorProfile() {
  const [selectedDayIndex, setSelectedDayIndex] = useState(2); // Mon is default
  ...
  const appointments = APPOINTMENTS_BY_DATE[selectedDate] || [];

  const goPrevDay = () => setSelectedDayIndex((i) => Math.max(i - 1, 0));
  const goNextDay = () =>
  ...
  const handleSendMessage = (e) => {
    ...
    // Replace with a real API call once the messaging endpoint exists
    console.log("Sending message to Dr. Terry:", messageText);
    setMessageText("");
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-6">
          <div className="w-full flex flex-col gap-6 mt-6">
              ...
              <div className="w-full h-[319px] flex items-start mt-6">
                ...
                <div className="w-[716px] h-[272px] shrink-0 flex gap-4 justify-between">
                  <div className="w-[169px] h-[272px] shrink-0 flex flex-col gap-[39px]">
                    ...
                    <InfoItem label="Address" value="Nasr City, Cairo, Egypt" />
                  </div>

                  <div className="w-[144.5px] h-[272px] shrink-0 flex flex-col gap-[43px]">
                    <InfoItem label="Date of Birth" value="Oct 12, 1999" />
                    <InfoItem label="Work Email" value={profile.email} />
                    <InfoItem label="Join Date" value="Oct 12, 1999" />
                  </div>

                  <div className="w-[144.5px] h-[272px] shrink-0 flex flex-col gap-[43px]">
                    <InfoItem label="Age" value="42 Years" />
                    <InfoItem label="Nationality" value="Egyptian" />
                    <InfoItem label="#ID" value="DOC-1001" />
                  </div>
            ...
            <div className="w-full max-w-[1128px] h-[522px] shrink-0 flex gap-[20px]">
              {/* Left Part — Professional Details */}
              <div className="flex-1 h-[522px] flex flex-col justify-between rounded-[24px] border border-[#DFE1E6] p-4">
                ...
                <DetailRow label="Department" value="Cardiology" />
                <DetailRow
                  label="Specialty"
                  value="Interventional Cardiologist"
                />
                <DetailRow label="Employment Type" value="Full-Time" />
                <DetailRow label="Years of Experience" value="12 Years" />
                <DetailRow label="Qualification" value="MBBS, MD Cardiology" />
                <DetailRow label="Medical License Number" value="MED-458932" />
              </div>

              {/* Right Part — Today's Patient */}
