...
const INITIAL_CONVERSATIONS = [
  {
    ...
    messages: [
      ...
      },
      {
        id: 2,
        sender: "them",
        text: "Hello Doctor, i'm feeling a lot better today, thank you",
        time: "8:00 PM",
      },
      {
        id: 3,
      ...
      },
      {
        id: 4,
        sender: "them",
        text: "yes , i take them as you advice.",
        time: "8:00 PM",
      },
      {
        id: 5,
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center justify-between gap-2">
          <p className="truncate text-xs text-slate-400">
            {lastMessage ? lastMessage.text.split("\n")[0] : ""}
          </p>
...
function StartConversationIllustration({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="flex w-full flex-col items-center gap-3 rounded-2xl px-4 py-6 text-center transition hover:bg-slate-50"
    >
      <svg
        width="120"
        height="90"
      ...
      <p className="text-xs text-slate-400">
        You can message your doctor anytime.
      </p>
    </button>
  );
}
...
function DateDivider({ label }) {
  return (
    <div className="my-4 flex justify-center">
      <span className="rounded-full bg-[#EEF2FF] px-3 py-1 text-xs font-medium text-slate-500">
        {label}
      </span>
    </div>
  );
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {conversation.messages.map((message, index) => {
          const showDivider =
            message.date &&
            (index === 0 ||
              conversation.messages[index - 1].date !== message.date);
          return (
...
function MessagesWorkspaceContent() {
  ...
  const handleStartConversation = () => {
    const name = window.prompt(
      "Enter the patient's name to start a new conversation",
    );
  ...
  return (
    <div className="flex h-full w-full">
      <div className="flex h-full w-[380px] shrink-0 flex-col gap-4 overflow-hidden border border-[#E5E7EB] p-4 box-border">
